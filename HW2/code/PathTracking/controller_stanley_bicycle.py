...
class ControllerStanleyBicycle(Controller):
    ...
    def feedback(self, info):
        ...
        theta_path = target[2]
        theta_e = utils.angle_norm(theta_path - yaw)
        
        dx = front_x - target[0]
        dy = front_y - target[1]
        
        theta_p_rad = np.deg2rad(theta_path)
        e = np.cos(theta_p_rad) * dy - np.sin(theta_p_rad) * dx
        
        delta_e = np.rad2deg(np.arctan2(-self.kp * e, max(vf, 0.001)))
        
        next_delta = utils.angle_norm(theta_e + delta_e)
        # [end] TODO 4.3.1
