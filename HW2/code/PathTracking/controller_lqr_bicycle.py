...
class ControllerLQRBicycle(Controller):
    ...
    def feedback(self, info):
        ...
        if self.control_state == 'steering_angle':
            ...
            v_val = max(v, 0.001)
            A = np.array([[1.0, -v_val * self.dt],
                          [0.0, 1.0]])
            B = np.array([[0.0],
                          [-v_val * self.dt / self.l]])

            theta_path = target[2]
            theta_e_rad = np.deg2rad(utils.angle_norm(theta_path - yaw))
            
            dx = x - target[0]
            dy = y - target[1]
            theta_path_rad = np.deg2rad(theta_path)
            e = np.cos(theta_path_rad) * dy - np.sin(theta_path_rad) * dx
            ...
            v_val = max(v, 0.001)
            A = np.array([[1.0, -v_val * self.dt, 0.0],
                          [0.0, 1.0, -v_val * self.dt / self.l],
                          [0.0, 0.0, 1.0]])
            B = np.array([[0.0],
                          [0.0],
                          [self.dt]])
            ...
            delta_dot_deg = np.rad2deg(u_k[0, 0])
            next_delta = delta + delta_dot_deg * self.dt
            # [end] TODO 4.4.4
