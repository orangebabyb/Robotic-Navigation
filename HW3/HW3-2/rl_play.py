...
class RewardManager:
    ...
    def calculate_distance_reward(self):
        ...
        # 如果這一幀剛剛拿到旗子，目標點會跳到下一個遠處，避開此時的距離變動懲罰
        ...
        # We can use the difference to provide continuous smooth reward.
        # It's positive when getting closer, negative when getting farther.
        diff = prev_dist - curr_dist
        
        # You could also use a constant like +0.1 and -0.1 if requested, but difference is the standard shaping reward
        if curr_dist < prev_dist:
            return float(diff * 5.0)  # Scale to make it a noticeable reward
        elif curr_dist > prev_dist:
            return float(diff * 5.0)  # Will be negative length penalty
    ...
    def calculate_reward(self):
        ...
        # 1. Time penalty to avoid wandering
        time_penalty = -0.02
        
        # 2. Water / mud penalty in 5x5 detection range
        water_penalty = 0.0
        if "terrain_grid" in self.observation:
            for i in range(5):
                for j in range(5):
                    cell = self.observation["terrain_grid"][i][j]
                    if cell["terrain_type"] == -1: # Water or mud
                        # Chebyshev distance from agent center (2, 2)
        ...
        total_reward = checkpoint_score + distance_score + survival_score + time_penalty + water_penalty
